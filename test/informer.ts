...
const listFn = async () => {
  const object = await k8sCustomObjectsApi.listClusterCustomObject(
    "x4.sap.com",
    "v1",
    "x4apps"
  );
...
informer.on("add", (obj: k8s.V1Pod) => {
  console.log(`Added: ${obj.metadata!.namespace}/${obj.metadata!.name}`);
});
informer.on("update", (obj: k8s.V1Pod) => {
  console.log(`Updated: ${obj.metadata!.namespace}/${obj.metadata!.name}`);
});
informer.on("delete", (obj: k8s.V1Pod) => {
  console.log(`Deleted: ${obj.metadata!.namespace}/${obj.metadata!.name}`);
});
